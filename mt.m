...
:- instance urandom(params, ustate) where [
    pred(generate_uint8/4) is mt.generate_uint8,
    pred(generate_uint16/4) is mt.generate_uint16,
    pred(generate_uint32/4) is mt.generate_uint32,
    pred(generate_uint64/4) is mt.generate_uint64
].

:- instance urandom_dup(ustate) where [
    pred(urandom_dup/3) is mt.urandom_dup
   ].
...
seed(SeedIn, S) :-
    seed(SeedIn, P, R),
    S = make_shared_random(P,R).

generate_uint8(_, N, !S) :-
    mt.generate_uint8(N, !S).

generate_uint16(_, N, !S) :-
    mt.generate_uint16(N, !S).

generate_uint32(_, N, !S) :-
    mt.generate_uint32(N, !S).

generate_uint64(_, N, !S) :-
    mt.generate_uint64(N, !S).
